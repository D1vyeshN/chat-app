...
import { X, LogOut, Shield, Users, Trash2, Edit2, Check, UserPlus, Search, Loader2 } from "lucide-react";
...
export default function InfoPanel({ room, onClose, onRoomUpdate }: InfoPanelProps) {
  ...
  useEffect(() => {
    if (showAddMember && searchQuery.trim()) {
      const search = async () => {
        setSearching(true);
        try {
          const { data } = await api.get<User[]>(`/api/users/search?query=${searchQuery}`);
          // Filter out users already in room
          setFoundUsers(data.filter(u => !room.members.find(m => m._id === u._id)));
        } catch (error) {
  ...
  const handleUpdateName = async () => {
    ...
    try {
      const { data } = await api.put(`/api/rooms/${room._id}`, { name: newName.trim() });
      onRoomUpdate(data);
      setIsEditingName(false);
    } catch (error) {
  ...
  const handleAddMember = async (userId: string) => {
    try {
      const { data } = await api.post(`/api/rooms/${room._id}/members`, { userId });
      onRoomUpdate(data);
  ...
  const handleRemoveMember = async (memberId: string) => {
    ...
    try {
      const { data } = await api.delete(`/api/rooms/${room._id}/members/${memberId}`);
      onRoomUpdate(data);
    } catch (error) {
      ...
      <ScrollArea className="flex-1">
        <div className="p-6">
          <div className="flex flex-col items-center mb-8">
            <Avatar className="h-24 w-24 rounded-2xl ring-2 ring-slate-800 ring-offset-4 ring-offset-slate-900 mb-4">
              <AvatarFallback className="bg-indigo-600 rounded-2xl text-2xl font-bold text-white capitalize">
            ...
            {isEditingName ? (
              <div className="flex items-center gap-2 w-full max-w-[200px]">
                <Input
                  ...
                  autoFocus
                />
                <Button size="icon" variant="ghost" onClick={handleUpdateName} className="h-8 w-8 text-green-500">
                  <Check className="h-4 w-4" />
                </Button>
                <Button size="icon" variant="ghost" onClick={() => setIsEditingName(false)} className="h-8 w-8 text-red-500">
                  <X className="h-4 w-4" />
                </Button>
              </div>
            ) : (
              <div className="flex items-center gap-2">
                ...
                {room.isGroup && isAdmin && (
                  <Button size="icon" variant="ghost" onClick={() => setIsEditingName(true)} className="h-6 w-6 text-slate-500 hover:text-white">
            ...
            {!room.isGroup && otherUser?.isOnline && (
              <span className="text-green-400 text-xs font-medium mt-1">Online</span>
            )}
            {room.isGroup && (
              ...
              <div className="space-y-4">
                ...
                {showAddMember && (
                  <div className="space-y-2 animate-in fade-in slide-in-from-top-2 duration-200">
                    ...
                    <div className="max-h-40 overflow-y-auto space-y-1 rounded-lg border border-slate-800 p-1">
                      {searching ? (
                        <div className="flex justify-center py-4"><Loader2 className="h-4 w-4 animate-spin text-slate-600" /></div>
                      ) : foundUsers.length === 0 ? (
                        ...
                        foundUsers.map(u => (
                          <button
                            ...
                            className="w-full flex items-center gap-2 p-1.5 rounded hover:bg-slate-800 transition-colors text-left"
                          >
                            <Avatar className="h-5 w-5"><AvatarFallback className="text-[8px] bg-slate-700">{u.username[0]}</AvatarFallback></Avatar>
                            <span className="text-xs text-slate-300 capitalize">{u.username}</span>
                          </button>
                        ))
                      )}
                    </div>
                      ...
                      <div className="flex items-center gap-3">
                        <div className="relative">
                          <Avatar className="h-8 w-8 rounded-lg">
                            <AvatarFallback className="bg-slate-800 rounded-lg text-xs font-bold text-white uppercase">
                              {member.username[0]}
                            </AvatarFallback>
                          </Avatar>
                          {member.isOnline && (
                        ...
                        <div>
                          <p className="text-sm text-slate-300 font-medium capitalize flex items-center gap-1.5">
                            ...
                            {member._id === room.createdBy && (
                              <Shield className="h-3 w-3 text-indigo-500" title="Admin" />
                            )}
                          </p>
                        </div>
                      </div>
